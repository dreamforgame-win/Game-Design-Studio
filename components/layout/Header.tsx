...
import { ChatAssistantWindow } from "@/components/ai/ChatAssistantWindow";
...
export function Header({
    ...
    try {
      ...
      const requestModels = () => {
        return new Promise<any[]>((resolve, reject) => {
          ...
          if (
            apiKey.trim() &&
            !cleanUrl.includes("generativelanguage.googleapis.com")
          ) {
              ...
              if (xhr.status === 200) {
                try {
                  ...
                  if (resp && Array.isArray(resp.data)) {
                    ...
                    const potentialArrays = Object.entries(resp).find(
          ...
          xhr.onerror = function () {
            reject(
              new Error(
                "XMLHttpRequest 网络请求或跨域错误。如果代理路由有 CORS 限制或拦截 GCP，建议开启插件模式或手动填入模型名称。",
              ),
            );
          };
      ...
      const cleanList = Array.from(new Set(list)).filter(
        (m) => m && typeof m === "string" && m.length < 100,
      );
      ...
      if (isAbortError) {
        ...
        displayMsg.includes("Failed to fetch")
      ) {
  ...
  const handleSaveConfig = () => {
    if (typeof window !== "undefined") {
      ...
      localStorage.setItem(
        "ai_manual_model_input",
        isManualModelInput ? "true" : "false",
      );
  ...
  return (
        ...
        >
          {mounted && resolvedTheme === "dark" ? (
            <Sun
              size={16}
              strokeWidth={2}
              className="animate-in spin-in-90 fade-in duration-300"
            />
          ) : (
            <Moon
              size={16}
        ...
        {currentView === "workspace" ? (
          <>
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-colors cursor-pointer ${showChat ? "bg-stone-100 dark:bg-white/10 border-stone-300 dark:border-white/20" : "border-stone-200 dark:border-white/10 hover:bg-stone-100 dark:hover:bg-white/5"}`}
            >
    ...
    {showChat && <ChatAssistantWindow onClose={() => setShowChat(false)} />}
        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-xs flex items-center justify-center z-[100000] pointer-events-auto animate-in fade-in duration-150">
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-white/10 rounded-2xl p-6 w-[430px] shadow-2xl flex flex-col gap-4 animate-in zoom-in-95 duration-150">
            <div>
              ...
              <p className="text-xs text-stone-500 dark:text-stone-400 leading-relaxed">
                填入大模型 API
                接口地址与密匙，一键载入模型池，提供文本与图片多模态分析支持。
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between mt-1 px-1">
                ...
                <label className="flex items-center gap-1.5 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-[10px] text-stone-500 dark:text-stone-400 font-medium font-sans">
                    手动输入模型名称
                  </span>
                </label>
              </div>
